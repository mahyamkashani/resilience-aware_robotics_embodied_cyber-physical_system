#!/bin/bash
#
# data_exp20.sh — non-critical-only attacks on navigate_to_goal (box2), NOT TOLERABLE.
#
# Attacks 2 non-critical devices (left_arm tau=1, right_arm tau=1). k_crit=0.
# alpha_base=0.20, theta_base=0.72, baseline=20 s. No mitigation.
...
#   1     left_arm  [non-critical]      1    ≈0.819   tolerable     (delta=1, gamma=1)
#   2     +right_arm    [non-critical]      2    ≈0.670   NOT tolerable (delta=1, gamma=0)
#   3     left_arm+right_arm            2    ≈0.670   sustained → HALTED at 40 s
#
# Outputs (written by manual_run.py):
#   exp_2a.csv       — summary row (appended each run)
#   exp_2a_psi.csv   — psi sampled at 100 Hz
#   exp_2a_delta.csv — delta sampled at 100 Hz
#
# Usage:
#   ./auto_runs/data_exp_2a.sh
#   ./auto_runs/data_exp_2a.sh configs/experiment_2a.json ../results/framework_correctness/exp_2a.csv
#
set -u
...
CONFIG="${1:-configs/experiment_2a.json}"
RESULT="${2:-../results/framework_correctness/exp_2a.csv}"

WAVE1="${WAVE1:-left_arm:STOP}"
WAVE2="${WAVE2:-left_arm:STOP,right_arm:STOP}"
