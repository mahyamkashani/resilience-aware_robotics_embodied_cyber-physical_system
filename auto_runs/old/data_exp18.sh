#!/bin/bash
#
# data_exp18.sh — wheel-only attacks on navigate_to_goal (box2), WITH mitigation.
#
...
#   2     left_wheels+right_wheels      2    ~0.741   NOT tolerable (delta=1, gamma=0)
#   ---   mitigation kicks in           0    1.000   recovered     (delta=1, gamma=1)

# Outputs (written by manual_run.py):
#   exp18.csv       -- summary row (appended each run)
#   exp18_psi.csv   -- psi sampled at 100 Hz
#   exp18_delta.csv -- delta sampled at 100 Hz
#
# Usage:
#   ./auto_runs/data_exp18.sh
#   ./auto_runs/data_exp18.sh configs/experiment18.json ../results/framework_correctness/exp18.csv
#
set -u
...
CONFIG="${1:-configs/experiment18.json}"
RESULT="${2:-../results/framework_correctness/exp18.csv}"
...
( sleep "$DELAY3"
  ...
  echo "  Mitigation pending: left_wheels + right_wheels will be recovered"
  ros2 topic pub --once /active_attacks my_attack_interfaces/msg/AttackState \
