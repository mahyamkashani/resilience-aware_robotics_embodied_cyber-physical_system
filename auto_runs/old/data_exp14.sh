#!/bin/bash
#
# data_exp14.sh — wheel-only attacks on navigate_to_goal (box2).
#
...
#   2     right_wheels:STOP            1       ≈0.923   tolerable
#   3     left_wheels + right_wheels   2       ≈0.852   tolerable
#   4     left_wheels + right_wheels   2       ≈0.852   sustained → HALTED at 40 s
#
# Outputs (written by manual_run.py):
#   exp14.csv       — summary row (appended each run)
#   exp14_psi.csv   — psi sampled at 100 Hz
#   exp14_delta.csv — delta sampled at 100 Hz
#
# Usage:
#   ./auto_runs/data_exp14.sh
#   ./auto_runs/data_exp14.sh configs/experiment14.json ../results/framework_correctness/exp14.csv
#
set -u
...
CONFIG="${1:-configs/experiment14.json}"
RESULT="${2:-../results/framework_correctness/exp14.csv}"
...
# --- inject attacks ----------------------------------------------------------
( sleep "$DELAY2"
  echo "[Wave 1 @ ${DELAY2}s]  $_yaml1  k_crit=1 → psi≈0.923"
