...
#   3     right_wheels+right_arm            2    ≈0.670   sustained → HALTED at 40 s
#
# Outputs (written by manual_run.py):
#   exp20.csv       — summary row (appended each run)
#   exp20_psi.csv   — psi sampled at 100 Hz
#   exp20_delta.csv — delta sampled at 100 Hz
#
# Usage:
#   ./auto_runs/data_exp20.sh
#   ./auto_runs/data_exp20.sh configs/experiment20.json ../results/framework_correctness/exp20.csv
#
set -u
...
CONFIG="${1:-configs/experiment20.json}"
RESULT="${2:-../results/framework_correctness/exp20.csv}"

WAVE1="${WAVE1:-right_wheels:STOP}"
WAVE2="${WAVE2:-right_wheels:STOP,right_arm:STOP}"
WAVE3="${WAVE3:-right_wheels:STOP,right_arm:STOP}"
